...
function createCommentElement(content, username, level = 0) {
  ...
  commentDiv.classList.add('p-4', 'rounded', 'shadow-md', 'mb-2', 'space-y-2');
  commentDiv.style.backgroundColor = '#f9eae1';
  ...
  replyButton.classList.add('text-blue-500', 'text-sm', 'mr-2', 'flex', 'items-center');
  replyButton.innerHTML = '<svg class="w-4 h-4 mr-1"><use href="#reply-icon"></use></svg>Reply';
  replyButton.addEventListener('click', () => addReplyInput(commentDiv, level + 1));

  const editButton = document.createElement('button');
  editButton.classList.add('text-yellow-500', 'text-sm', 'mr-2', 'flex', 'items-center');
  editButton.innerHTML = '<svg class="w-4 h-4 mr-1"><use href="#edit-icon"></use></svg>Edit';
  editButton.addEventListener('click', () => toggleEdit(commentText, commentInput, editButton, saveButton));

  const saveButton = document.createElement('button');
  saveButton.classList.add('text-green-500', 'text-sm', 'mr-2', 'flex', 'items-center', 'hidden');
  saveButton.innerHTML = '<svg class="w-4 h-4 mr-1"><use href="#save-icon"></use></svg>Save';
  saveButton.addEventListener('click', () => saveEdit(commentText, commentInput, editButton, saveButton));

  const deleteButton = document.createElement('button');
  deleteButton.classList.add('text-red-500', 'text-sm', 'flex', 'items-center');
  deleteButton.innerHTML = '<svg class="w-4 h-4 mr-1"><use href="#delete-icon"></use></svg>Delete';
...
function addReplyInput(parentComment, level) {
  ...
  replyDiv.style.backgroundColor = '#e6f3f5';
